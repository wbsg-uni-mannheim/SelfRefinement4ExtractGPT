...
def update_task_dict_from_validation_set(task_dict, challenging=False):
    """Update task dict from validation set."""
    directory_path = f'{PROCESSED_DATASETS}/{task_dict["dataset_name"]}/'
    if challenging:
        file_path = os.path.join(directory_path, 'challenging_validation.jsonl')
    else:
        file_path = os.path.join(directory_path, 'validation.jsonl')
    task_dict['known_attributes'] = {}
...
def update_task_dict_from_train_set(task_dict, train_percentage=1.0):
    """Update task dict from test set."""
    directory_path = f'{PROCESSED_DATASETS}/{task_dict["dataset_name"]}/'
...
def load_known_attribute_values(dataset_name, n_examples=5, consider_casing=False, train_percentage=1.0, test_set=False, with_function=False):
    ...
    with open(file_path, 'r') as f:
        for line in f.readlines():
            ...
            for attribute in record['target_scores']:
                ...
                if with_function and 'split_target_scores' in record and attribute in record['split_target_scores']:
                    value = json.dumps(record['split_target_scores'][attribute])
