...
def calculate_evaluation_metrics_multiple_attributes(targets, preds, categories, unique_category,
    ...
    for target, pred, category in zip(targets, preds, categories):
        ...
        try:
            ...
            prediction = prediction.strip() if prediction != "n/a" and prediction != "None" else None

        except Exception as e:
            print(e)
            print('Not able to decode prediction: \n {} \n and attribute: {}'.format(pred, unique_attribute))
            prediction = None
...
def calculate_recall_precision_f1(targets, preds, categories, attributes):
    ...
    for unique_category, unique_attribute in product(set(categories), set(attributes)):
        eval_dict = calculate_evaluation_metrics(targets, preds, categories, attributes, unique_category,
                                                 unique_attribute)
...
def calculate_precision_recall_f1(eval_dict):
    ...
    """
    precision = round((eval_dict[VC] / (eval_dict[NV] + eval_dict[VC] + eval_dict[VW])) * 100, 2) if (eval_dict[NV] +
                                                                                                      eval_dict[VC] +
                                                                                                      eval_dict[
                                                                                                          VW]) > 0 else 0
    recall = round((eval_dict[VC] / (eval_dict[VN] + eval_dict[VC] + eval_dict[VW])) * 100, 2) if (eval_dict[VN] +
                                                                                                   eval_dict[VC] +
                                                                                                   eval_dict[
                                                                                                       VW]) > 0 else 0
...
def calculate_micro_scores(total_eval):
    ...
    """
    micro_precision = round((total_eval[VC] / (total_eval[NV] + total_eval[VC] + total_eval[VW])) * 100, 2) if (
                                                                                                                           total_eval[
                                                                                                                               NV] +
                                                                                                                           total_eval[
                                                                                                                               VC] +
                                                                                                                           total_eval[
                                                                                                                               VW]) > 0 else 0
    micro_recall = round((total_eval[VC] / (total_eval[VN] + total_eval[VC] + total_eval[VW])) * 100, 2) if (total_eval[
                                                                                                                 VN] +
                                                                                                             total_eval[
                                                                                                                 VC] +
                                                                                                             total_eval[
