...
def main(dataset, model, verbose, description_configuration, no_example_values, shots, train_percentage):
    # Load task template
    with open('prompts/task_template.json', 'r') as f:
        task_dict = json.load(f)

    # Assign file name to task_dict
    task_dict['task_name'] = f'08_fs_extraction_{shots}__ex_val_{description_configuration}_desc_{no_example_values}_output_feedback_run_0'
    ...
    with open('prompts/prompt_templates/08_few-shot_extraction_example_values_output_feedback/sys_extraction.txt',
              'r') as f:
        system_prompt_txt = f.read()

    with open('prompts/prompt_templates/08_few-shot_extraction_example_values_output_feedback/sys_review.txt',
              'r') as f:
