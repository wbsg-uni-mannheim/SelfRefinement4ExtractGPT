...
def main(dataset, model, verbose, description_configuration, no_example_values, train_percentage):
    ...
    task_dict['task_name'] = f'02_zs_extraction_ex_val_{description_configuration}_desc_{no_example_values}_run_0'
    ...
    #task_dict = update_task_dict_from_train_set(task_dict)
    ...
    with open('prompts/prompt_templates/02_zero-shot_extraction_example_values/sys_extraction.txt',
              'r') as f:
    ...
    for i in range(3):
        ...
        with get_openai_callback() as cb:
            ...
            for example in tqdm(task_dict['examples']):
                ...
                formatted_attribute_descriptions = '\n'.join([f"{attribute}: {attribute_descriptions[attribute]}" for attribute in attributes
                                                              if attribute in attribute_descriptions and len(attribute_descriptions[attribute]) > 0])
                ...
                response = llm_chain.invoke({
                                        ...
                                        "attribute_descriptions": formatted_attribute_descriptions,
                                        "input": example['input']
                                        })
