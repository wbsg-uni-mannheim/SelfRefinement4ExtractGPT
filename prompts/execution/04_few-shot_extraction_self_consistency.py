...
@click.option('--temperature', default=0.0, help='Temperature for sampling in self-consistency')
def main(dataset, model, verbose, shots, train_percentage, temperature):
    # Load task template
    with open('prompts/task_template.json', 'r') as f:
        task_dict = json.load(f)

    # Assign file name to task_dict
    task_dict['task_name'] = f'04_fs_extraction_shots_{shots}_self_consistency_temperature_{temperature}_run_0'
    ...
    with open('prompts/prompt_templates/04_few-shot_extraction_self_consistency/sys_extraction.txt',
              'r') as f:
    ...
    for i in range(3):
        ...
        with get_openai_callback() as cb:
            ...
            for example in tqdm(task_dict['examples']):
                ...
                # Do three runs of the same task
                ...
                for j in range(3):
                    ...
                    # Shuffle the attributes for the next run
                    random.shuffle(attributes)

                # Do majority voting for self-consistency
                response = {}
                for attribute in attributes:
                    attribute_values = [str(response[attribute]) for response in self_consistency_responses if attribute in response]
                    if len(attribute_values) > 0:
                        response[attribute] = max(set(attribute_values), key=attribute_values.count)
